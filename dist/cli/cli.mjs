...
function parseArguments(pArguments) {
    ...
    if (values["dot-graph-attrs"])
        values["dot-graph-attrs"] = validations.validDotAttrs(values["dot-graph-attrs"]);
    if (values["dot-node-attrs"])
        values["dot-node-attrs"] = validations.validDotAttrs(values["dot-node-attrs"]);
    if (values["dot-edge-attrs"])
        values["dot-edge-attrs"] = validations.validDotAttrs(values["dot-edge-attrs"]);
...
export default async function cli(pArguments = process.argv, pOptions) {
    ...
    try {
        ...
        await transform(validations.validateArguments(normalize(positionals[0], values)));
    }
    catch (pError) {
