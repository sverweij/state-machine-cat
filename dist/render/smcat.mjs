...
function stateHasExtendedAttributes(pState) {
    return (Object.hasOwn(pState, "label") ||
...
function actions(pActions) {
    return pActions
        .map((pAction) => `${pAction.type === "activity" ? "" : `${pAction.type}/ `}${pAction.body}`)
...
function transitionHasExtendedAttributes(pTransition) {
    return Object.entries(pTransition).some(([pKey]) => RENDERABLE_TRANSITION_ATTRIBUTES.includes(pKey));
}
...
export default function renderSmcat(pStateMachine, _pOptions = {}, pIndent = "") {
